import { Link } from "react-router-dom";
import BrowseImage from "./browse-image/browse-image";
import AddImageToCollection from "./add-image-to-collections/add-image-to-collection";
import "./card.css";
export default function Card(props) {
  return (
    <div className="card" id={props.id}>
      <div className="image-area">
        <img className="image" alt="Изображение" src={props.image.imageFile === null ? props.image.link : "data:image/*;base64," + props.image.imageFile} />
        <div className="button-layout" key={props.id.replace("image", "button-layer")}>
          <BrowseImage data={props} />
          <div className="author-area">
            <Link to={`/@${props.author.userName}`}>
              {props.author.photoLink === null ? (
                <svg className="author-button" viewBox="3 3 18 18" fill="none">
                  <path
                    d="M12.12 12.78C12.05 12.77 11.96 12.77 11.88 12.78C10.12 12.72 8.71997 11.28 8.71997 9.50998C8.71997 7.69998 10.18 6.22998 12 6.22998C13.81 6.22998 15.28 7.69998 15.28 9.50998C15.27 11.28 13.88 12.72 12.12 12.78Z"
                    stroke="black"
                    strokeWidth="0.7"
                    fill="black"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M18.74 19.3801C16.96 21.0101 14.6 22.0001 12 22.0001C9.40001 22.0001 7.04001 21.0101 5.26001 19.3801C5.36001 18.4401 5.96001 17.5201 7.03001 16.8001C9.77001 14.9801 14.25 14.9801 16.97 16.8001C18.04 17.5201 18.64 18.4401 18.74 19.3801Z"
                    stroke="black"
                    fill="black"
                    strokeWidth="0.7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" stroke="black" strokeWidth="0.7" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              ) : (
                <img src={props.author.photoLink} className="author-button" alt="Фото автора" />
              )}
            </Link>
            <Link to={`/@${props.author.userName}`} className="author-title">
              {props.author.userName}
            </Link>
          </div>
          {props.status === "authorized" && (
            <button className="collection-button">
              <AddImageToCollection data={props} />
              <svg width="20px" height="20px" viewBox="0 0 24 21">
                <path d="m22 12a1 1 0 0 1 -1 1h-8v8a1 1 0 0 1 -2 0v-8h-8a1 1 0 0 1 0-2h8v-8a1 1 0 0 1 2 0v8h8a1 1 0 0 1 1 1z" />
              </svg>
            </button>
          )}
        </div>
      </div>
      <div className="tags-area">
        {props.tags.slice(0, 5).map((tag, index) => (
          <button
            className="tag"
            key={index}
            onClick={() => {
              props.onTagClick(tag);
            }}
          >
            {tag}
          </button>
        ))}
      </div>
    </div>
  );
}
