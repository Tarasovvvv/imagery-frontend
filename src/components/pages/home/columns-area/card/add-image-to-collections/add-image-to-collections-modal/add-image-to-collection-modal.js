    ...
    >
      <div style={{ padding: "0", margin: "0", display: "flex", height: "100%" }}>
        ...
        >
          <div
            key="-2"
            style={{ cursor: "pointer", lineHeight: "2.5", textAlign: "center", marginBottom: "10px", borderRadius: "5px", height: "60px", fontSize: "22px" }}
            className="collection-button-modal"
            onClick={async () => {
              const response = await fetch(api.addCollection, {
                method: "POST",
                body: JSON.stringify(props.data.image.id),
                credentials: "include",
              });
